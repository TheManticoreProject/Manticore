...
func TestPad(t *testing.T) {
	...
	for i, test := range PadTests {
		...
		copy(buf, test.in)
		pad, err := pkcs7.Pad(buf, aes.BlockSize)
		if err != nil {
			t.Errorf("[%d] %v", i, err)
		}
...
func TestUnpadErrors(t *testing.T) {
	...
	for i, test := range BadPadTests {
		_, err := pkcs7.Unpad(test)
		if err != pkcs7.ErrInvalidPadding {
			t.Errorf("[%d] expected %v, got %v", i, pkcs7.ErrInvalidPadding, err)
		}
	}
}
...
func completelyUnsafeNotConstantTimeUnpad(buf []byte) ([]byte, error) {
	if len(buf) == 0 {
		return nil, pkcs7.ErrUnPaddingEmptyBuffer
	}
	padLen := buf[len(buf)-1]
	if int(padLen) > len(buf) || padLen == 0 {
		return nil, pkcs7.ErrInvalidPadding
	}
...
func TestUnpadBlackBox(t *testing.T) {
	t.Parallel()
	err := quick.CheckEqual(pkcs7.Unpad, completelyUnsafeNotConstantTimeUnpad, nil)
	if err != nil {
		t.Error(err)
	}
}
