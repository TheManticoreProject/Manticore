...
func (c *LockAndReadResponse) Marshal() ([]byte, error) {
	...
	// Marshalling data BufferType
	rawDataContent = append(rawDataContent, types.UCHAR(c.BufferType))
...
func (c *LockAndReadResponse) Unmarshal(data []byte) (int, error) {
	...
	// Unmarshalling data BufferType
	if len(rawDataContent) < offset+1 {
	    return offset, fmt.Errorf("rawParametersContent too short for BufferType")
	}
	c.BufferType = types.UCHAR(rawDataContent[offset])
	offset++
