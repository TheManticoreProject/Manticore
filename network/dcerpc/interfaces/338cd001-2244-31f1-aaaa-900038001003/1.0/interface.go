...
// Status codes are not declared here. The winreg methods return error_status_t, a Win32
// error code ([MS-ERREF] 2.2 / [MS-RRP] 3.1.5), and the whole of that table already lives
// in github.com/TheManticoreProject/Manticore/windows/errors/win32 as the win32.WIN32_ERROR
// type. A subset repeated here would cover a fraction of the 2703-code table and drift
// from it, so callers convert the returned status with win32.WIN32_ERROR(status) and
// compare it against win32.ERROR_SUCCESS and the other codes of that package.
