...
type r_WinsTermRequest struct {
	FAbruptTem int16
}

func (*r_WinsTermRequest) Opnum() uint16 { return winsif.OpnumR_WinsTerm }

// r_WinsTermResponse carries the [out] parameters and return value of R_WinsTerm.
type r_WinsTermResponse struct {
	Status ndr.DWORD `ndr:"retval"`
}

// R_WinsTerm calls R_WinsTerm (opnum 6) ([MS-RAIW] — verify the parameter
// modeling and status handling).
func R_WinsTerm(rpc ndr.Invoker, fAbruptTem int16) (err error) {
	req := &r_WinsTermRequest{
		FAbruptTem: fAbruptTem,
	}
	var resp r_WinsTermResponse
