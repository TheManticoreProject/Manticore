...
type iDL_DRSInitDemotionRequest struct {
	...
	PmsgIn      msdrsr.DRS_MSG_INIT_DEMOTIONREQ
}
...
type iDL_DRSInitDemotionResponse struct {
	...
	PmsgOut       msdrsr.DRS_MSG_INIT_DEMOTIONREPLY
	Status        ndr.DWORD `ndr:"retval"`
}

// IDL_DRSInitDemotion calls IDL_DRSInitDemotion (opnum 25) ([MS-DRSR] — verify the parameter
// modeling and status handling).
func IDL_DRSInitDemotion(rpc ndr.Invoker, hDrs msdrsr.DRS_HANDLE, dwInVersion ndr.DWORD, pmsgIn msdrsr.DRS_MSG_INIT_DEMOTIONREQ) (PdwOutVersion ndr.DWORD, PmsgOut msdrsr.DRS_MSG_INIT_DEMOTIONREPLY, err error) {
	...
	if win32.WIN32_ERROR(resp.Status) != win32.ERROR_SUCCESS {
		err = fmt.Errorf("IDL_DRSInitDemotion failed: %s", win32.WIN32_ERROR(resp.Status).String())
	}
	return
}
