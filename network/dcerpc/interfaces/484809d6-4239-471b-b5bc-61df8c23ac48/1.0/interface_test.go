...
// TestSyntaxID pins the abstract syntax identifier for the TermSrvSession interface
// (484809d6-4239-471b-b5bc-61df8c23ac48 v1.0, [MS-TSTS]).
func TestSyntaxID(t *testing.T) {
	s := SyntaxID()
	if got := s.UUID.ToFormatD(); got != "484809d6-4239-471b-b5bc-61df8c23ac48" {
		t.Errorf("UUID = %s, want 484809d6-4239-471b-b5bc-61df8c23ac48", got)
	}
...
func TestStatusCodesResolveThroughHRESULT(t *testing.T) {
	...
	// E_NOTIMPL was outside the subset this interface used to declare, so it rendered as
	// hex; it resolves by name now.
	if got := hresult.HRESULT(0x80004001).String(); got != "E_NOTIMPL" {
		t.Errorf("hresult.HRESULT(0x80004001).String() = %q, want E_NOTIMPL", got)
	}
