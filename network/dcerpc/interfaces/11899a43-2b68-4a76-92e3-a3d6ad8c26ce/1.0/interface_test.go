...
// TestSyntaxID pins the abstract syntax identifier for the TermSrvNotification interface
// (11899a43-2b68-4a76-92e3-a3d6ad8c26ce v1.0, [MS-TSTS]).
func TestSyntaxID(t *testing.T) {
	s := SyntaxID()
	if got := s.UUID.ToFormatD(); got != "11899a43-2b68-4a76-92e3-a3d6ad8c26ce" {
		t.Errorf("UUID = %s, want 11899a43-2b68-4a76-92e3-a3d6ad8c26ce", got)
	}
...
// TestStatusCodesResolveThroughHRESULT pins that the HRESULT values [MS-TSTS] 3.3.4.2
...
func TestStatusCodesResolveThroughHRESULT(t *testing.T) {
	...
	// RpcWaitForSessionState and RpcWaitAsyncNotification both take a timeout and
	// report WAIT_TIMEOUT when it expires. That value is FACILITY_WIN32, so it is
	// HRESULT_FROM_WIN32 of the Win32 code ([MS-ERREF] 2.1.2). It was outside the
	// subset this interface used to declare, so it rendered as hex; it resolves by
	// name now.
	if got := hresult.FromWin32(win32.WAIT_TIMEOUT); uint32(got) != 0x80070102 {
		t.Errorf("hresult.FromWin32(WAIT_TIMEOUT) = 0x%08x, want 0x80070102", uint32(got))
	}
	if got := hresult.HRESULT(0x80070102).String(); got != "HRESULT_FROM_WIN32(WAIT_TIMEOUT)" {
		t.Errorf("hresult.HRESULT(0x80070102).String() = %q, want HRESULT_FROM_WIN32(WAIT_TIMEOUT)", got)
	}
