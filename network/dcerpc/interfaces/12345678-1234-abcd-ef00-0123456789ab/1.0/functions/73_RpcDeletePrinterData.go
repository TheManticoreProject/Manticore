...
func RpcDeletePrinterData(rpc ndr.Invoker, hPrinter msrprn.PRINTER_HANDLE, pValueName ndr.WSTR) (err error) {
	...
	if win32.WIN32_ERROR(resp.Status) != win32.ERROR_SUCCESS {
		err = fmt.Errorf("RpcDeletePrinterData failed: %s", win32.WIN32_ERROR(resp.Status).String())
	}
	return
}
