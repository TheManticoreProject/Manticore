...
type rpcGetCorePrinterDriversResponse struct {
	PCorePrinterDrivers []structures.CORE_PRINTER_DRIVER `ndr:"ref,size_is=CCorePrinterDrivers"`
	Status              ndr.DWORD                        `ndr:"retval"`
}

// RpcGetCorePrinterDrivers calls RpcGetCorePrinterDrivers (opnum 102) ([MS-RPRN] — verify the parameter
// modeling and status handling).
func RpcGetCorePrinterDrivers(rpc ndr.Invoker, pszServer *ndr.WSTR, pszEnvironment ndr.WSTR, cchCoreDrivers ndr.DWORD, pszzCoreDriverDependencies []uint16, cCorePrinterDrivers ndr.DWORD) (PCorePrinterDrivers []structures.CORE_PRINTER_DRIVER, err error) {
