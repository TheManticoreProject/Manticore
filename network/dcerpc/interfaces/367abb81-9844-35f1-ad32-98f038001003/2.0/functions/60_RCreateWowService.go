...
func RCreateWowService(rpc ndr.Invoker, hSCManager msscmr.SC_RPC_HANDLE, lpServiceName ndr.WSTR, lpDisplayName *ndr.WSTR, dwDesiredAccess ndr.DWORD, dwServiceType ndr.DWORD, dwStartType ndr.DWORD, dwErrorControl ndr.DWORD, lpBinaryPathName ndr.WSTR, lpLoadOrderGroup *ndr.WSTR, lpdwTagId *ndr.DWORD, lpDependencies []uint8, dwDependSize ndr.DWORD, lpServiceStartName *ndr.WSTR, lpPassword []uint8, dwPwSize ndr.DWORD, dwServiceWowType uint16) (LpdwTagId *ndr.DWORD, LpServiceHandle msscmr.LPSC_RPC_HANDLE, err error) {
	...
	if win32.WIN32_ERROR(resp.Status) != win32.ERROR_SUCCESS {
		err = fmt.Errorf("RCreateWowService failed: %s", win32.WIN32_ERROR(resp.Status).String())
	}
	return
}
