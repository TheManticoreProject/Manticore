...
// TestSyntaxID pins the abstract syntax identifier for the RCMPublic interface
// (bde95fdf-eee0-45de-9e12-e5a61cd0d4fe v1.0, [MS-TSTS]).
func TestSyntaxID(t *testing.T) {
	s := SyntaxID()
	if got := s.UUID.ToFormatD(); got != "bde95fdf-eee0-45de-9e12-e5a61cd0d4fe" {
		t.Errorf("UUID = %s, want bde95fdf-eee0-45de-9e12-e5a61cd0d4fe", got)
	}
...
func TestStatusCodesResolveThroughHRESULT(t *testing.T) {
	...
	// E_UNEXPECTED was outside the subset this interface used to declare, so it rendered
	// as hex; it resolves by name now.
	if got := hresult.HRESULT(0x8000FFFF).String(); got != "E_UNEXPECTED" {
		t.Errorf("hresult.HRESULT(0x8000ffff).String() = %q, want E_UNEXPECTED", got)
	}

	// A FACILITY_WIN32 failure [MS-ERREF] 2.1.1 does not name resolves through the Win32
	// code it wraps, which is where the rest of the 0x8007XXXX space now comes from.
