...
func NetrShareDelCommit(rpc *client.Client, contextHandle structures.SHARE_DEL_HANDLE) (structures.SHARE_DEL_HANDLE, error) {
	req := &netrShareDelCommitRequest{
		ContextHandle: contextHandle,
	}
	var resp netrShareDelCommitResponse
	if err := rpc.Invoke(req, &resp); err != nil {
		return structures.SHARE_DEL_HANDLE{}, fmt.Errorf("NetrShareDelCommit: %w", err)
	}
