...
// (drsrtypes.DS_NAME_* — DS_NAME_NO_ERROR means resolved), the crossRef domain, and the
...
func (c *Client) CrackNames(formatOffered, formatDesired uint32, names ...string) ([]CrackedName, error) {
	...
	msgIn := drsrtypes.DRS_MSG_CRACKREQ{
		Tag: 1,
		V1: drsrtypes.DRS_MSG_CRACKREQ_V1{
...
// drsrtypes.DS_NT4_ACCOUNT_NAME for "DOMAIN\\user") to its objectGUID by cracking it to
...
func (c *Client) ResolveToGUID(accountName string, formatOffered uint32) (guid.GUID, error) {
	results, err := c.CrackNames(formatOffered, drsrtypes.DS_UNIQUE_ID_NAME, accountName)
	if err != nil {
	...
	r := results[0]
	if r.Status != drsrtypes.DS_NAME_NO_ERROR {
