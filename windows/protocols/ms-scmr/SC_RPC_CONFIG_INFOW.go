...
// SC_RPC_CONFIG_INFOW ([MS-SCMR] 2.2.22) carries an info level plus a [switch_is(dwInfoLevel)]
// union selecting one configuration record. dwInfoLevel is also re-transmitted inline as the
// union discriminant ([C706] 14.3.8).
type SC_RPC_CONFIG_INFOW struct {
	DwInfoLevel ndr.DWORD
	Field       SC_RPC_CONFIG_INFOW_Field
}

// SC_RPC_CONFIG_INFOW_Field is the [switch_is(dwInfoLevel)] union of SC_RPC_CONFIG_INFOW.
