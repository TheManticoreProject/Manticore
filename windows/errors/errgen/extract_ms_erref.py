#!/usr/bin/env python3
"""Extract the [MS-ERREF] Win32 error code table into the TSV that errgen reads.

The Win32 error codes are published only as an HTML table on Microsoft Learn, so
the table is extracted once and the result committed next to this script. That
keeps generation offline and makes a change to the spec show up as a diff in the
TSV rather than as a change in this parser.

Usage:
    python3 extract_ms_erref.py > ms-erref-2.2-win32.tsv          # fetch and extract
    python3 extract_ms_erref.py page.html > ms-erref-2.2-win32.tsv # extract a local copy
...
def extract(page):
    ...
    if len(out) < 2700:
        raise SystemExit(f"only {len(out)} rows extracted; the page layout has changed")
    return out
...
def main():
    ...
    print("# [MS-ERREF] 2.2 Win32 Error Codes, extracted by extract_ms_erref.py.")
    print(f"# Source: {URL}")
